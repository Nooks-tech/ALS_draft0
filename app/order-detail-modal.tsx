...
export default function OrderDetailModal() {
  ...
  useEffect(() => {
    if (!orderId || order?.status !== 'Delivered' || !order?.merchantId) return;
    setLoadingComplaint(true);
    getOrderComplaint(orderId, order.merchantId).then((c) => {
      setExistingComplaint(c);
      setLoadingComplaint(false);
    }).catch(() => setLoadingComplaint(false));
  }, [orderId, order?.status, order?.merchantId]);
      ...
      <View className="absolute bottom-0 left-0 right-0 bg-white rounded-t-[40px] max-h-[85%] overflow-hidden">
        ...
        <ScrollView className="flex-1" contentContainerStyle={{ paddingHorizontal: 24, paddingVertical: 16 }} showsVerticalScrollIndicator={false}>
          ...
          {(() => {
            ...
            const Row = ({ label, amount, negative = false, bold = false }: { label: string; amount: number; negative?: boolean; bold?: boolean }) => (
              <View className="flex-row justify-between items-center py-1">
                <Text className={bold ? 'font-bold text-slate-800' : 'text-slate-600 text-sm'}>{label}</Text>
                <View className="flex-row items-center">
                  {negative ? <Text className={bold ? 'font-bold text-emerald-600' : 'text-emerald-600 text-sm'}>−</Text> : null}
                  <PriceWithSymbol
                    amount={amount}
                    iconSize={bold ? 16 : 14}
                    iconColor={negative ? '#059669' : bold ? primaryColor : '#475569'}
                    textStyle={{
                      color: negative ? '#059669' : bold ? primaryColor : '#475569',
