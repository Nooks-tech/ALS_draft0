...
export const OrdersProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const addOrder = useCallback(
    (
      ...
      setOrders((prev) => {
        // Dedupe by id — the foreground refresh/poll (refresh() →
        // fetchOrdersForCustomer) can surface the same row that addOrder
        // is inserting optimistically, so both can land for one id in a
        // race. If the fetch gets there first the array already contains
        // this order, so "next = [placed, ...prev]" would show it twice
        // with different statuses (optimistic vs server). Filter first.
