 ...
 *
 * INPUT-2 (2026-07-10 audit): use Express's proxy-aware req.ip rather than
 * the leftmost X-Forwarded-For token. The server runs under `trust proxy: 1`
 * (server/index.ts), so Express derives req.ip from XFF using only the single
 * trusted hop — a client that prepends its own spoofed XFF entries can't
 * shift the key (the leftmost-token read WAS client-spoofable, letting an
 * attacker rotate the value to reset the per-IP window). Fall back to the
 * raw socket address if req.ip is somehow unavailable.
 */
