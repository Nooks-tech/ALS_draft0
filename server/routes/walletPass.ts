 ...
 * Self-contained: forge for hashing/signing, do-not-zip for archive.
 * No passkit-generator (its decryptRsaPrivateKey fails on PKCS#8 keys).
 */
...
function signManifest(manifestBuf: Buffer): Buffer {
  ...
  p7.content = new forge.util.ByteStringBuffer(manifestBuf.toString('binary'));
...
walletPassRouter.get('/wallet-pass/debug', (_req, res) => {
  ...
  try {
    ...
    const wwdr = forge.pki.certificateFromPem(decode(WWDR_BASE64!).toString('utf-8'));
    info.wwdrCN = wwdr.subject.getField('CN')?.value;

    const testManifest = Buffer.from('{"test":"1"}');
    ...
    info.testSigCerts = p7.certificates?.length ?? 0;

    const rc = p7.rawCapture || {};
    const signerInfosRaw = rc.signerInfos;
    info.testSigHasSignerInfos = !!(signerInfosRaw && signerInfosRaw.length > 0);
  } catch (e: any) {
...
walletPassRouter.get('/wallet-pass', async (req, res) => {
  try {
    ...
    files['pass.json'] = makePassJson({
      ...
      cardLabel, points, lifetimePoints, pointValueSar,
    ...
    console.error('[WalletPass]', err?.message);
