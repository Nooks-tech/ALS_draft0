...
/* ─── PRIV-02: Saved-card Moyasar token encryption at rest ───────────────────
   ...
   The two other token readers — server/cron/savedCardSweep.ts +
   server/routes/wallet.ts — import decryptSavedCardToken from this module.)
 ...
 *
 * Idempotent on (customer_id, merchant_id, token_hash) — re-attaching
 * the same token returns the existing card row instead of duplicating.
 * (PRIV-02: the token column is encrypted with a per-write random IV,
 * so the stable sha256 token_hash is the dedup key.)
 */
/**
