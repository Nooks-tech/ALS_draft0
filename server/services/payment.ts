...
export const paymentService = {
  ...
  async initiateMoyasarTokenPayment(req: PaymentInitRequest & { token: string }): Promise<PaymentSession> {
    ...
    const body: Record<string, unknown> = {
      ...
      // NOTE: no save_card flag here. save_card is only documented for
      // creditcard/applepay sources, not token sources, and the token we
      // charge is a verified-active reusable token (default /v1/tokens
      // flow) whose lifetime is until card expiry — nothing to extend.
      ...(req.orderId ? { given_id: orderIdToUuid(req.orderId) } : {}),
